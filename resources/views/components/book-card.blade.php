...
<a href="{{ route('books.show', $book) }}"
    class="block bg-white rounded shadow-sm hover:shadow-md transition overflowhidden">
    {{-- Portada --}}
    @if($book->cover_url)
        <img src="{{ $book->cover_url }}" alt="{{ $book->title }}" class="w-full aspect-[2/3] object-cover">
    @else
            <div class="w-full aspect-[2/3] bg-slate-200 flex items-center justifycenter
         text-slate-400">
